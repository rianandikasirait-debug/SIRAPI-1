...
<div class="max-w-xl mx-auto mt-10 bg-white p-6 rounded-xl shadow">
    ...
    <form action="{{ route('sekolah.update', $sekolah->id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label for="nama_sekolah" class="block mb-1">Nama Sekolah</label>
            <input id="nama_sekolah" type="text" name="nama_sekolah" value="{{ old('nama_sekolah', $sekolah->nama_sekolah) }}" class="w-full border p-2 rounded">
        </div>

        <div>
            <label for="alamat" class="block mb-1">Alamat</label>
            <textarea id="alamat" name="alamat" class="w-full border p-2 rounded">{{ old('alamat', $sekolah->alamat) }}</textarea>
        </div>

        <div>
            <label for="telepon" class="block mb-1">Telepon</label>
            <input id="telepon" type="text" name="telepon" value="{{ old('telepon', $sekolah->telepon) }}" class="w-full border p-2 rounded">
        </div>
